<template>
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(address,index) in addressShowList" v-bind:key="address.addressId" v-bind:class="{'check':currAddress==index}" @click="setCurrAddress(index)">
                  <dl>
                    <dt>{{address.userName}}</dt>
                    <dd class="address">{{address.streetName}}</dd>
                    <dd class="tel">{{address.tel}}</dd>
                  </dl>
                  <div class="addr-opration addr-del" @click="deleteConfirm(address.addressId)">
            ...
            <div class="shipping-addr-more" v-if="addressList.length>limitShow" @click="limitShow=99">
    ...
    <modal v-bind:mdShow="mdShow" v-on:closeModal="closeModalWindow">
      <p slot="message">
        You have not login, please login first.
      </p>
      <div slot="btnGroup">
        <a class="btn btn--m" @click="closeModalWindow">Close</a> 
      </div>
    </modal>
    <modal v-bind:mdShow="confirmShow" v-on:closeModal="closeModalWindow">
      <p slot="message">
        Do you confirm delete this address?
      </p>
      <div slot="btnGroup">
        <a class="btn btn--m" @click="deleteAddress">Yes</a>
        <a class="btn btn--m" @click="closeModalWindow">No</a>
      </div>
    </modal>
...
export default {
  ...
  computed: {
    addressShowList() {
      return this.addressList.slice(0, this.limitShow);
    }
  },
  methods: {
    getAddressList() {
      axios.post("/users/getAddressList").then(resp => {
        let res = resp.data;
        if (res.status == "0") {
    ...
    deleteAddress() {
      this.confirmShow = false;
      console.log("post delete address: id = " + this.deleteAddressId);
    }
  }
};
</script>
