        ...
        <div class="checkout-order">
          ...
          <div class="item-list-wrap confirm-item-list-wrap">
            <div class="cart-item order-item">
              ...
              <ul class="cart-item-list">
                <li v-for="item in orderList" v-bind:key="item.producrId">
          ...
          <div class="order-foot-wrap">
            <div class="prev-btn-wrap">
              <button class="btn btn--m" @click="pre">Previous</button>
            </div>
            <div class="next-btn-wrap">
              <button class="btn btn--m btn--red" @click="next">Proceed to payment</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    totalPrice() {
      ...
      this.tax = totalPrice * this.taxRate;

      totalPrice += this.tax + this.shipPrice;
      return totalPrice;
    },
    subTotalPrice() {
      let subTotalPrice = 0;
      this.orderList.forEach(item => {
        subTotalPrice += item.productNum * item.salePrice;
      });
  ...
  methods: {
    init() {
      axios.post("/users/getCartList").then(resp => {
        let res = resp.data;
        if (res.status == "0") {
          console.log("init success");
          this.orderList = res.result.filter(item => item.checked == "1");
    ...
    next() {
      // TODO save the order
      axios.post("/users/createOrder", {orederList: this.orderList}).then(resp => {
        let res = resp.data;
        if (res.status == "0") {
